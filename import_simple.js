...
async function importWithRawMongoDB() {
  try {
    ...
    try {
      ...
      // Delete existing electricity law documents using raw MongoDB
      ...
      // Create document object without schema validation
      ...
      // Insert using raw MongoDB operation
      const insertResult = await collection.insertOne(electricityLaw);
      ...
      const savedDoc = await collection.findOne({ titleArabic: /كهرباء/ });
      if (savedDoc) {
        ...
        if (article27Saved) {
          ...
          console.log('Article 27 content length:', article27Saved.content.length);
        } else {
